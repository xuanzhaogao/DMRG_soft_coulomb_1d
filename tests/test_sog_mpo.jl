...
function test_small()
    ...
    # SOG (with QN)
    @printf "[SOG+QN] Building MPO... "
    flush(stdout)
    t2 = @elapsed (H_s, sites_s) = build_hamiltonian_sog(L, delta, rc, z, r; N=N)
    @printf "%.2f s (bond dim=%d)\n" t2 maxlinkdim(H_s)
    flush(stdout)
    ...
    @printf "[SOG] E = %.10f, ⟨Ntot⟩=%.4f, ⟨Sz⟩=%.4f\n" E_s ntot sz
    flush(stdout)

    diff = abs(E_s - E_n)
    @printf "\nΔE = %.2e  (SOG kernel error ~5e-4)\n" diff
    @printf "Status: %s\n" (diff < 5e-3 ? "PASS" : "FAIL")
end
...
function test_profile()
    ...
    @printf "[SOG] DMRG (5 sweeps, χ=200)...\n"
    flush(stdout)
